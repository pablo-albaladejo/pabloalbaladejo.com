...
- **llms.txt format is audited.** `public/llms.txt` follows the [llms.txt spec](https://llmstxt.org/): a single H1, a `>` blockquote summary, then H2 sections whose entries are markdown links (`- [Name](https://url): description`). Lighthouse's "Agent Accessibility" audit rejects bare URLs ("File does not appear to contain any links") — keep every listed URL in markdown link form.
